...
<body>
  ...
  <form>
    ...
    def get_json_params_schema(self) -> Dict[str, Any]:
        return {
            ...
            "params": [
                ...
                {"name": "words", "type": "list[str]", "default": ["APSI", "INVENIRA", "FACADE", "ADAPTER", "PROXY"]},
            ],
        }

    def resolve_user_url(self, activity_id: str, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        entry_url = f"{self.base_url}/game/{activity_id}"
        if user_id:
            entry_url = f"{self.base_url}/game/{activity_id}?userID={user_id}"

        return {"activityID": activity_id, "entry_url": entry_url, "instance_id": instance_id}

    def list_analytics(self) -> List[Dict[str, Any]]:
        return [
            {"id": "access_count", "label": "Total de acessos (por atividade)", "method": "POST analytics", "params": ["activityID"]},
            {"id": "events_count", "label": "Total de eventos (por atividade)", "method": "POST analytics", "params": ["activityID"]},
            {"id": "user_events_count", "label": "Eventos por aluno (activityID+userID)", "method": "POST analytics", "params": ["activityID", "userID"]},
        ]
    ...
    def query_analytics(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        events = self.proxy.list_events(activity_id=activity_id, user_id=user_id)
        ...
        if query in ("default", "events_count"):
            result["events_count"] = len(self.proxy.list_events(activity_id=activity_id))
